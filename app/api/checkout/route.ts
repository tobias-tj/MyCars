...
export async function POST(
  ...
  const numberOrDays = Math.ceil(
    (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)
  );

  const totalAmount = Number(priceDay) * numberOrDays;
  const totalAmountStripe = Math.round(totalAmount * 100); // Multiplica por 100 para convertir a centavos
